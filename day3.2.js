...
const [points1, points2] = paths
  .map(path => {
    const points = path.split(',').reduce((acc, instruction) => {
      ...
      const newPoints = operations[direction](parseInt(instruction.slice(1)), lastPos || '0,0')
        .filter(p => !acc.includes(p))
...
const setPoints2 = new Set(points2)

const [closestPoint] = points1.filter(point => setPoints2.has(point))

console.log(
  points1.indexOf(closestPoint) + points2.indexOf(closestPoint) + 2
)
