...
function makeArray (length, fill) {
  return Array.from({ length }).fill(
    fill.split(',').map(x => parseInt(x))
  )
}

const operations = {
  U: (distance, pos) => makeArray(distance, pos).map(([x, y], i) => x + ',' + (y + i + 1)),
  D: (distance, pos) => makeArray(distance, pos).map(([x, y], i) => x + ',' + (y - i - 1)),
  R: (distance, pos) => makeArray(distance, pos).map(([x, y], i) => (x + i + 1) + ',' + y),
  L: (distance, pos) => makeArray(distance, pos).map(([x, y], i) =>  (x - i - 1) + ',' + y)
}

console.time()

const [points1, points2] = paths
  .map(path => {
    const points = path.split(',').reduce((acc, instruction) => {
      ...
      return [
        ...acc,
        ...operations[direction](parseInt(instruction.slice(1)), lastPos || '0,0')
      ]
    }, [])
