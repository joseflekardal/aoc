function findValidPasswords({ start, end }) {
  ...
  for (let password = start; password < end; password++) {
    ...
    const isIncreasing = digits.every(
      (digit, i) => i + 1 === digits.length || digits[i + 1] >= digit
    )
    ...
    if (hasDoubles && hasDoubles[0].length > 2) {
      ...
      doubleIsNotPartOfLargerGroup =
        remainsHasDoubles && remainsHasDoubles[0].length === 2
    }
...
console.log(findValidPasswords({ start: 136818, end: 685979 }))
